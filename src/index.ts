...
// The v3.2 dream composer, LLM consolidation and LLM reflection were removed here on
// 21 Sep 2026 (Interior Build Plan 1.0). Rule of 24 Jun 2026: eyes, never the pen —
// no small/cheap model authors journals, observations or relational memory in this mind.
// The daemon below is mechanics only. Git history has the bodies if ever wanted for reference.
...
async function processSubconscious(env: Env): Promise<void> {
  ...
  try {
    ...
    // processSubconscious. Consolidation becomes flag-only in a later step.
